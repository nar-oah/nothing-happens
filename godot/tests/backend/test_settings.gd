...
func _test_settings_commands(t: BackendTestContext) -> void:
	...
	t.check(saved["snapshot"] == JSON.parse_string(before), "a game save after settings changes contains the same run snapshot")
...
func _test_quit_autosave(t: BackendTestContext) -> void:
	...
	t.check(bridge.save_at_quit["snapshot"] == JSON.parse_string(expected), "quit writes the complete current term before exiting")
