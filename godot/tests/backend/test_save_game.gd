...
func _check_references(t: BackendTestContext, loaded: RunSession, control: RunSession) -> void:
	...
	t.check(state.draft_bill.policies[0].definition == Fixture.Policy, "draft policy resolves its canonical definition resource")
	t.check_equal(state.draft_bill.policies[0].delay_months, 7, "draft policy keeps its selected delay")
	t.check(state.saved_bills[0].policies[0] != state.draft_bill.policies[0], "saved and draft policy instances stay independent")
	t.check_equal(state.saved_bills[0].policies[0].delay_months, 8, "saved policy keeps its selected delay")
	t.check(state.scheduled_policies[0] == state.active_bill.policies[0], "scheduled policy shares the enacted bill instance")
	t.check(state.scheduled_policies[0].definition == Fixture.Policy, "scheduled policy resolves its canonical definition resource")
	t.check_equal(state.scheduled_policies[0].delay_months, 5, "scheduled policy keeps its locked delay")
	t.check_equal(state.scheduled_policies[0].elapsed_months, 4, "scheduled policy keeps elapsed settlement months")
