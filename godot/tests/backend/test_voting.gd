...
func _test_peach_weighted_race_vote(t: BackendTestContext) -> void:
	...
	t.check_equal(tied.support_count, 0, "Peach seats provide no support when internal weight is not over half")
	t.check_equal(tied.abstain_count, 3, "all present Peach seats abstain when internal support lacks a majority")
	...
	t.check_equal(reduced.support_count, 2, "Peach consensus makes every present Peach seat support")
	t.check_equal(reduced.absent_count, 1, "absent Peach seats remain absent instead of collapsing into one race vote")
