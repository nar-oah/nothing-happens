...
func _test_game_root_shell(t: BackendTestContext) -> void:
	...
	t.check(root.has_node("SettingsManager"), "GameRoot owns settings separately from RunSession")
	...
	t.check(bridge.settings_manager == root.get_node("SettingsManager"), "production bridge receives SettingsManager")
	var ready := bridge.receive_ipc_message(_message("ui.ready", {}))
