...
<button class="flex h-full w-full flex-col gap-[3px] overflow-hidden border-0 bg-transparent px-8 py-5 text-left" type="button" aria-label="进入次月" {disabled} onmouseenter={() => setHovering(true)} onmouseleave={() => setHovering(false)} onfocus={() => setHovering(true)} onblur={() => setHovering(false)} onclick={onAdvance}>
	...
	<div class="flex w-full shrink-0 items-start gap-12 overflow-hidden text-ink-primary">
		<div class="flex min-w-0 flex-1 flex-col items-start overflow-hidden">
			<p class="typo-newspaper-masthead w-full"><MorphText text={hovering && !disabled ? '次月' : '弦外'} /></p>
