...
<button class="flex h-full w-full flex-col gap-[3px] overflow-hidden border-0 bg-transparent px-8 py-5 text-left" type="button" aria-label="进入次月" onmouseenter={() => (hovering = true)} onmouseleave={() => (hovering = false)} onfocus={() => (hovering = true)} onblur={() => (hovering = false)} onclick={onAdvance}>
	...
	<div class="flex w-full shrink-0 items-start gap-12 overflow-hidden text-ink-primary">
		...
		<div class="flex h-[72px] w-[95px] shrink-0 flex-col items-start gap-px overflow-hidden border-l border-ink-primary pl-8 text-center">
			<p class="typo-newspaper-data-hero w-full"><MorphText text={monthText} /></p>
			<div class="typo-newspaper-caption flex w-full items-center justify-center whitespace-nowrap"><MorphText text={`${monthKind}月 · 第${displayYear}年`} /></div>
		</div>
	</div>
	<div class="typo-newspaper-caption flex w-full shrink-0 items-start gap-4 overflow-hidden bg-surface-indigo px-[6px] py-2 text-surface-amber whitespace-nowrap">
		<span>鉛字報館</span><span>·</span><span>VOL.<MorphText text={String(displayYear)} /></span><span>NO.<MorphText text={monthText} /></span>
	</div>
</button>
