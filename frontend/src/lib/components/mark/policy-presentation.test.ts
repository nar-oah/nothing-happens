import assert from 'node:assert/strict';
import test from 'node:test';
import { Metric, PolicyEffectFormula, type PolicyDefinition } from '../../game/types.ts';
import { translate, type Translate } from '../../i18n/index.ts';
import { policyToMemorialContent } from '../memorial/presentation.ts';
import { createPolicyMarkContent } from './mark.ts';

const zh: Translate = (key, params) => translate(key, params, 'zh_CN');
const en: Translate = (key, params) => translate(key, params, 'en');

const policy: PolicyDefinition = {
	display_name: '测试政策',
	effects: [
		{
			target_metric: Metric.PRODUCTION,
			formula: PolicyEffectFormula.METRIC_GAP,
			source_a: Metric.PRODUCTION,
			source_b: Metric.CONSUMPTION,
			multiplier: -0.5
		},
		{
			target_metric: Metric.TAX,
			formula: PolicyEffectFormula.METRIC_GAP,
			source_a: Metric.PRODUCTION,
			source_b: Metric.CONSUMPTION,
			multiplier: 1
		}
	]
};

const baseline = {
	tax: 100,
	consumption: 100,
	production: 150,
	employment: 100,
	investment: 100
};

test('policy mark faces show the tier-scaled gap effect separately from smoothing effects', () => {
	assert.deepEqual(createPolicyMarkContent(policy, baseline, zh), {
		gap: { label: '落差', headline: '税課－50', detail: '消費－生産' },
		smoothing: { label: '平抑', headline: '生産＋25', detail: '（消費－生産）×0.5' }
	});
	assert.equal(createPolicyMarkContent(policy, baseline, en).smoothing.label, 'Damp');
});

test('vertical memorial policy content separates the tier-scaled gap effect from smoothing', () => {
	assert.deepEqual(policyToMemorialContent(policy, zh, zh), {
		policyTitle: '测试政策',
		contents: [
			{ title: '落差', body: '税課＋（消費－生産）' },
			{ title: '平抑', body: '生産－（消費－生産）×0.5' }
		]
	});
	assert.deepEqual(
		policyToMemorialContent(policy, en, en).contents.map((content) => content.title),
		['Gap', 'Damp']
	);
});

test('effects other than the tier-scaled gap effect appear in the smoothing section in order', () => {
	const withThirdEffect: PolicyDefinition = {
		...policy,
		effects: [
			...policy.effects,
			{
				target_metric: Metric.EMPLOYMENT,
				formula: PolicyEffectFormula.METRIC_VALUE,
				source_a: Metric.TAX,
				source_b: Metric.TAX,
				multiplier: -0.25
			}
		]
	};
	assert.deepEqual(createPolicyMarkContent(withThirdEffect, baseline, zh).smoothing, {
		label: '平抑',
		headline: '生産＋25\n就業－25',
		detail: '（消費－生産）×0.5\n税課×0.25',
		lines: [
			{ headline: '生産＋25', detail: '（消費－生産）×0.5' },
			{ headline: '就業－25', detail: '税課×0.25' }
		]
	});
	assert.equal(
		policyToMemorialContent(withThirdEffect, zh, zh).contents[1].body,
		'生産－（消費－生産）×0.5\n就業－税課×0.25'
	);
});
